...
export default function SellerOrdersPage() {
  ...
  const loadOrders = async () => {
    setLoading(true);
    try {
      const res = await sellerApi.getOrders({ page, per_page: 15 });
      setOrders(res.data);
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {loading ? (
                ...
                <tr><td colSpan={5} className="py-8 text-center text-gray-500">Aucune commande</td></tr>
              ) : (
                orders.map(order => {
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4">
                        <Link href={`/seller/orders/${order.id}`} className="font-medium text-blue-600 hover:underline">#{order.order_number}</Link>
                      </td>
