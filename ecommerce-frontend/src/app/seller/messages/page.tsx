            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <p className="font-medium text-sm text-gray-900 truncate">{otherName || 'Utilisateur'}</p>
                  {(c.unread_count ?? 0) > 0 && (
                    <span className="bg-blue-600 text-white text-xs rounded-full px-2 py-0.5">{c.unread_count}</span>
                  )}
                </div>
                {c.last_message && (
                  <p className="text-xs text-gray-500 truncate mt-1">{c.last_message.content}</p>
                )}
                {c.product && (
